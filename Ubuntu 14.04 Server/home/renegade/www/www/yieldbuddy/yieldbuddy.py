...
def drawInterface():
	...
	print("\033[40;0H[  Lastest Messages  ]-------------------------------------------------------------------------------------")
...
def drawInterfaceIPs():
		...
		while i < len(IP_Addresses):
			print("\033[" + str(i+6) + ";80H")
			print("\033[" + str(i+6) + ";80H" + str(i+1) + ")" + str(IP_Addresses[i]))	
			i=i+1
...
def checkSerial():
	...
	try:
		...
		global oldSetPoint_TankTotal
		global oldSetPoint_Tank1
		global oldSetPoint_Tank2
		global oldSetPoint_Tank3
		global oldSetPoint_Tank4
		global oldSetPoint_WaterTempP1
		global oldSetPoint_WaterTempP2
		global oldSetPoint_WaterTempP3
		global oldSetPoint_WaterTempP4
		...
		oldSetPoint_Light = " "
		oldSetPoint_Water = " "
		oldSetPoint_TankTotal = " "
		oldSetPoint_Tank1 = " "
		oldSetPoint_Tank2 = " "
		oldSetPoint_Tank3 = " "
		oldSetPoint_Tank4 = " "
		oldSetPoint_WaterTempP1 = " "
		oldSetPoint_WaterTempP2 = " "
		oldSetPoint_WaterTempP3 = " "
		oldSetPoint_WaterTempP4 = " "		
	...
	#Read Serial Device and Update according to the information that was received.
		...
		if 'Time' in line:
			...
			if ArduinoTime != '':
				...
				print("\033[9;0H                                                                                                                       ")
				print '\033[1m' #Bold
				print("\033[9;0H[Arduino Time: " + ArduinoTime + "]------------------------[Raspberry Pi Time: " + now.strftime("%b %d %Y %I:%M:%S %p")+"]")
				print '\033[0m' #Un-Bold
		...
		#If the 'timesync' counter value goes over 20, then update the Raspberry Pi's time to be that of the Arduino's.
			...
			if oldRelays != line:
				...
				print("\033[11;0H                                                                                                                       ")
				print("\033[11;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Relays: %s,%s,%s,%s,%s,%s,%s,%s"%(Relay1,Relay2,Relay3,Relay4,Relay5,Relay6,Relay7,Relay8))
				#RELAYS
			...
			if oldRelay_isAuto != line:
				...
				print("\033[12;0H                                                                                                                       ")
				print("\033[12;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Relay_isAuto: %s,%s,%s,%s,%s,%s,%s,%s"%(Relay1_isAuto,Relay2_isAuto,Relay3_isAuto,Relay4_isAuto,Relay5_isAuto,Relay6_isAuto,Relay7_isAuto,Relay8_isAuto))
				#RELAYS
			...
			if oldLight_Schedule != line:
				...
				print("\033[13;0H                                                                                                                       ")
				print("\033[13;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Light_Schedule: %s,%s,%s,%s"%(Light_ON_hour,Light_ON_min,Light_OFF_hour,Light_OFF_min))
				#LIGHTING
			...
			if oldWatering_Schedule != line:
				...
				print("\033[14;0H                                                                                                                       ")
				print("\033[14;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") Watering_Schedule: %s,%s,%s,%s,%s,%s,%s"%(Pump_start_hour,Pump_start_min,Pump_start_isAM,Pump_every_hours,Pump_every_mins,Pump_for,Pump_times))
				#WATERING
			...
			if oldSetPoint_pH1 != line:
				...
				print("\033[15;0H                                                                                                                       ")
				print("\033[15;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_pH1: %s,%s,%s"%(pH1Value_Low,pH1Value_High,pH1_Status))
				#SetPoint_pH
		...
		elif 'SetPoint_WaterTempP1' in line:
			if oldSetPoint_WaterTempP1 != line:
				oldSetPoint_WaterTempP1 = line
				#print("%s"%(line))  For Debugging
				SetPoint_WaterTempP1,WaterTempP1Value_Low,WaterTempP1Value_High,WaterTempP1Heater_ON,WaterTempP1Heater_OFF,WaterTempP1AC_ON,WaterTempP1AC_OFF,WaterTempP1_Status=line.split(",")
				SetPoint_WaterTempP1 = SetPoint_WaterTempP1.replace("Read fail", "")
				WaterTempP1_Status = WaterTempP1_Status.rstrip()
				print("\033[29;0H                                                                                                                       ")
				print("\033[29;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_WaterTempP1: %s,%s,%s,%s,%s,%s,%s"%(WaterTempP1Value_Low,WaterTempP1Value_High,WaterTempP1Heater_ON,WaterTempP1Heater_OFF,WaterTempP1AC_ON,WaterTempP1AC_OFF,WaterTempP1_Status))
				#SetPoint_WaterTempP1
				update_sql("UPDATE `WaterTempP1` SET Low = " + WaterTempP1Value_Low + ", High = " + WaterTempP1Value_High + ", WaterTempP1Heater_ON = " + WaterTempP1Heater_ON + ", WaterTempP1Heater_OFF = " + WaterTempP1Heater_OFF + ", WaterTempP1AC_ON =" + WaterTempP1AC_ON + ", WaterTempP1AC_OFF =" + WaterTempP1AC_OFF + ", Status ='" + WaterTempP1_Status + "'")
				WaterTempP1_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM WaterTempP1")
				WaterTempP1_High_Alarm = fetch_sql("SELECT High_Alarm FROM WaterTempP1")
				if WaterTempP1_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"WaterTempP1 Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `WaterTempP1` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if WaterTempP1_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"WaterTempP1 High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `WaterTempP1` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in WaterTempP1_Status and WaterTempP1_Low_Alarm[0] == 0:
					update_sql("UPDATE `WaterTempP1` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in WaterTempP1_Status and WaterTempP1_High_Alarm[0] == 0:
					update_sql("UPDATE `WaterTempP1` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")					
		elif 'SetPoint_WaterTempP2' in line:
			if oldSetPoint_WaterTempP2 != line:
				oldSetPoint_WaterTempP2 = line
				#print("%s"%(line))  For Debugging
				SetPoint_WaterTempP2,WaterTempP2Value_Low,WaterTempP2Value_High,WaterTempP2Heater_ON,WaterTempP2Heater_OFF,WaterTempP2AC_ON,WaterTempP2AC_OFF,WaterTempP2_Status=line.split(",")
				SetPoint_WaterTempP2 = SetPoint_WaterTempP2.replace("Read fail", "")
				WaterTempP2_Status = WaterTempP2_Status.rstrip()
				print("\033[30;0H                                                                                                                       ")
				print("\033[30;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_WaterTempP2: %s,%s,%s,%s,%s,%s,%s"%(WaterTempP2Value_Low,WaterTempP2Value_High,WaterTempP2Heater_ON,WaterTempP2Heater_OFF,WaterTempP2AC_ON,WaterTempP2AC_OFF,WaterTempP2_Status))
				#SetPoint_WaterTempP2
				update_sql("UPDATE `WaterTempP2` SET Low = " + WaterTempP2Value_Low + ", High = " + WaterTempP2Value_High + ", WaterTempP2Heater_ON = " + WaterTempP2Heater_ON + ", WaterTempP2Heater_OFF = " + WaterTempP2Heater_OFF + ", WaterTempP2AC_ON =" + WaterTempP2AC_ON + ", WaterTempP2AC_OFF =" + WaterTempP2AC_OFF + ", Status ='" + WaterTempP2_Status + "'")
				WaterTempP2_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM WaterTempP2")
				WaterTempP2_High_Alarm = fetch_sql("SELECT High_Alarm FROM WaterTempP2")
				if WaterTempP2_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"WaterTempP2 Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `WaterTempP2` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if WaterTempP2_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"WaterTempP2 High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `WaterTempP2` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in WaterTempP2_Status and WaterTempP2_Low_Alarm[0] == 0:
					update_sql("UPDATE `WaterTempP2` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in WaterTempP2_Status and WaterTempP2_High_Alarm[0] == 0:
					update_sql("UPDATE `WaterTempP2` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")					
		elif 'SetPoint_WaterTempP3' in line:
			if oldSetPoint_WaterTempP3 != line:
				oldSetPoint_WaterTempP3 = line
				#print("%s"%(line))  For Debugging
				SetPoint_WaterTempP3,WaterTempP3Value_Low,WaterTempP3Value_High,WaterTempP3Heater_ON,WaterTempP3Heater_OFF,WaterTempP3AC_ON,WaterTempP3AC_OFF,WaterTempP3_Status=line.split(",")
				SetPoint_WaterTempP3 = SetPoint_WaterTempP3.replace("Read fail", "")
				WaterTempP3_Status = WaterTempP3_Status.rstrip()
				print("\033[31;0H                                                                                                                       ")
				print("\033[31;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_WaterTempP3: %s,%s,%s,%s,%s,%s,%s"%(WaterTempP3Value_Low,WaterTempP3Value_High,WaterTempP3Heater_ON,WaterTempP3Heater_OFF,WaterTempP3AC_ON,WaterTempP3AC_OFF,WaterTempP3_Status))
				#SetPoint_WaterTempP3
				update_sql("UPDATE `WaterTempP3` SET Low = " + WaterTempP3Value_Low + ", High = " + WaterTempP3Value_High + ", WaterTempP3Heater_ON = " + WaterTempP3Heater_ON + ", WaterTempP3Heater_OFF = " + WaterTempP3Heater_OFF + ", WaterTempP3AC_ON =" + WaterTempP3AC_ON + ", WaterTempP3AC_OFF =" + WaterTempP3AC_OFF + ", Status ='" + WaterTempP3_Status + "'")
				WaterTempP3_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM WaterTempP3")
				WaterTempP3_High_Alarm = fetch_sql("SELECT High_Alarm FROM WaterTempP3")
				if WaterTempP3_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"WaterTempP3 Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `WaterTempP3` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if WaterTempP3_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"WaterTempP3 High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `WaterTempP3` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in WaterTempP3_Status and WaterTempP3_Low_Alarm[0] == 0:
					update_sql("UPDATE `WaterTempP3` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in WaterTempP3_Status and WaterTempP3_High_Alarm[0] == 0:
					update_sql("UPDATE `WaterTempP3` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")					
		elif 'SetPoint_WaterTempP4' in line:
			if oldSetPoint_WaterTempP4 != line:
				oldSetPoint_WaterTempP4 = line
				#print("%s"%(line))  For Debugging
				SetPoint_WaterTempP4,WaterTempP4Value_Low,WaterTempP4Value_High,WaterTempP4Heater_ON,WaterTempP4Heater_OFF,WaterTempP4AC_ON,WaterTempP4AC_OFF,WaterTempP4_Status=line.split(",")
				SetPoint_WaterTempP4 = SetPoint_WaterTempP4.replace("Read fail", "")
				WaterTempP4_Status = WaterTempP4_Status.rstrip()
				print("\033[32;0H                                                                                                                       ")
				print("\033[32;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_WaterTempP4: %s,%s,%s,%s,%s,%s,%s"%(WaterTempP4Value_Low,WaterTempP4Value_High,WaterTempP4Heater_ON,WaterTempP4Heater_OFF,WaterTempP4AC_ON,WaterTempP4AC_OFF,WaterTempP4_Status))
				#SetPoint_WaterTempP4
				update_sql("UPDATE `WaterTempP4` SET Low = " + WaterTempP4Value_Low + ", High = " + WaterTempP4Value_High + ", WaterTempP4Heater_ON = " + WaterTempP4Heater_ON + ", WaterTempP4Heater_OFF = " + WaterTempP4Heater_OFF + ", WaterTempP4AC_ON =" + WaterTempP4AC_ON + ", WaterTempP4AC_OFF =" + WaterTempP4AC_OFF + ", Status ='" + WaterTempP4_Status + "'")
				WaterTempP4_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM WaterTempP4")
				WaterTempP4_High_Alarm = fetch_sql("SELECT High_Alarm FROM WaterTempP4")
				if WaterTempP4_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"WaterTempP4 Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `WaterTempP4` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if WaterTempP4_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"WaterTempP4 High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `WaterTempP4` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in WaterTempP4_Status and WaterTempP4_Low_Alarm[0] == 0:
					update_sql("UPDATE `WaterTempP4` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in WaterTempP4_Status and WaterTempP4_High_Alarm[0] == 0:
					update_sql("UPDATE `WaterTempP4` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
			...
			if oldSetPoint_RH != line:
				...
				print("\033[18;0H                                                                                                                       ")
				print("\033[18;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_RH: %s,%s,%s,%s,%s,%s,%s"%(RHValue_Low,RHValue_High,Humidifier_ON,Humidifier_OFF,Dehumidifier_ON,Dehumidifier_OFF,RH_Status))
				#SetPoint_RH
			...
			if oldSetPoint_TDS1 != line:
				...
				print("\033[19;0H                                                                                                                       ")
				print("\033[19;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_TDS1: %s,%s,%s,%s,%s,%s"%(TDS1Value_Low,TDS1Value_High,NutePump1_ON,NutePump1_OFF,MixPump1_Enabled,TDS1_Status))
				#SetPoint_TDS1
			...
			if oldSetPoint_TDS2 != line:
				...
				print("\033[20;0H                                                                                                                       ")
				print("\033[20;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_TDS2: %s,%s,%s,%s,%s,%s"%(TDS2Value_Low,TDS2Value_High,NutePump2_ON,NutePump2_OFF,MixPump2_Enabled,TDS2_Status))
				#SetPoint_TDS2
			...
			if oldSetPoint_CO2 != line:
				...
				print("\033[21;0H                                                                                                                       ")
				print("\033[21;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_CO2: %s,%s,%s,%s,%s,%s"%(CO2Value_Low,CO2Value_High,CO2_ON,CO2_OFF,CO2_Enabled,CO2_Status))
				#SetPoint_CO2'
			...
			if oldSetPoint_Light != line:
				...
				print("\033[22;0H                                                                                                           ")
				print("\033[22;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_Light: %s,%s,%s"%(LightValue_Low,LightValue_High,Light_Status))
				#SetPoint_pH
			...
			if oldSetPoint_Water != line:
				...
				print("\033[23;0H                                                                                                                       ")
				print("\033[23;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_Water: %s,%s,%s"%(WaterValue_Low,WaterValue_High,Water_Status))
				#SetPoint_pH
				update_sql("UPDATE `Water` SET Low='" + WaterValue_Low + "',High='" + WaterValue_High + "',Status='" + Water_Status + "'")
				Water_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM Water")
				Water_High_Alarm = fetch_sql("SELECT High_Alarm FROM Water")
				if Water_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Water Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Water` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if Water_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Water High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Water` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in Water_Status and Water_Low_Alarm[0] == 0:
					update_sql("UPDATE `Water` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in Water_Status and Water_High_Alarm[0] == 0:
					update_sql("UPDATE `Water` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
			...
			if oldSetPoint_TankTotal != line:
				...
				print("\033[24;0H                                                                                                                       ")
				print("\033[24;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_TankTotal: %s,%s,%s"%(TankTotalValue_Low,TankTotalValue_High,TankTotal_Status))
				#SetPoint_TankTotal
				update_sql("UPDATE `TankTotal` SET Low='" + TankTotalValue_Low + "', High='" + TankTotalValue_High + "', Status='" + TankTotal_Status + "'")
				TankTotal_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM TankTotal")
				TankTotal_High_Alarm = fetch_sql("SELECT High_Alarm FROM TankTotal")
				if TankTotal_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"TankTotal Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `TankTotal` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if TankTotal_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"TankTotal High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `TankTotal` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in TankTotal_Status and TankTotal_Low_Alarm[0] == 0:
					update_sql("UPDATE `TankTotal` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in TankTotal_Status and TankTotal_High_Alarm[0] == 0:
					update_sql("UPDATE `TankTotal` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")							
			...
			if oldSetPoint_Tank1 != line:
				...
				print("\033[25;0H                                                                                                                       ")
				print("\033[25;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_Tank1: %s,%s,%s,%s,%s,%s"%(Tank1Value_Low,Tank1Value_High,Tank1Pump_ON,Tank1Pump_OFF,Tank1MixPump_Enabled,Tank1_Status))
				#SetPoint_Tank1
				update_sql("UPDATE `Tank1` SET Low = " + Tank1Value_Low + ", High = " + Tank1Value_High + ", Tank1Pump_ON = " + Tank1Pump_ON + ", Tank1Pump_OFF = " + Tank1Pump_OFF + ", Tank1MixPump_Enabled =" + Tank1MixPump_Enabled + ", Status ='" + Tank1_Status + "'")
				Tank1_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM Tank1")
				Tank1_High_Alarm = fetch_sql("SELECT High_Alarm FROM Tank1")
				if Tank1_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Tank1 Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Tank1` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if Tank1_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Tank1 High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Tank1` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in Tank1_Status and Tank1_Low_Alarm[0] == 0:
					update_sql("UPDATE `Tank1` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in Tank1_Status and Tank1_High_Alarm[0] == 0:
					update_sql("UPDATE `Tank1` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")					
			...
			if oldSetPoint_Tank2 != line:
				...
				print("\033[26;0H                                                                                                                       ")
				print("\033[26;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_Tank2: %s,%s,%s,%s,%s,%s"%(Tank2Value_Low,Tank2Value_High,Tank2Pump_ON,Tank2Pump_OFF,Tank2MixPump_Enabled,Tank2_Status))
				#SetPoint_Tank1
				update_sql("UPDATE `Tank2` SET Low = " + Tank2Value_Low + ", High = " + Tank2Value_High + ", Tank2Pump_ON = " + Tank2Pump_ON + ", Tank2Pump_OFF = " + Tank2Pump_OFF + ", Tank2MixPump_Enabled =" + Tank2MixPump_Enabled + ", Status ='" + Tank2_Status + "'")
				Tank2_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM Tank2")
				Tank2_High_Alarm = fetch_sql("SELECT High_Alarm FROM Tank2")
				if Tank2_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Tank2 Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Tank2` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if Tank2_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Tank2 High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Tank2` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in Tank2_Status and Tank2_Low_Alarm[0] == 0:
					update_sql("UPDATE `Tank2` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in Tank2_Status and Tank2_High_Alarm[0] == 0:
					update_sql("UPDATE `Tank2` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")										
			...
			if oldSetPoint_Tank3 != line:
				...
				print("\033[27;0H                                                                                                                       ")
				print("\033[27;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_Tank3: %s,%s,%s,%s,%s,%s"%(Tank3Value_Low,Tank3Value_High,Tank3Pump_ON,Tank3Pump_OFF,Tank3MixPump_Enabled,Tank3_Status))
				#SetPoint_Tank3
				update_sql("UPDATE `Tank3` SET Low = " + Tank3Value_Low + ", High = " + Tank3Value_High + ", Tank3Pump_ON = " + Tank3Pump_ON + ", Tank3Pump_OFF = " + Tank3Pump_OFF + ", Tank3MixPump_Enabled =" + Tank3MixPump_Enabled + ", Status ='" + Tank3_Status + "'")
				Tank3_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM Tank3")
				Tank3_High_Alarm = fetch_sql("SELECT High_Alarm FROM Tank3")
				if Tank3_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Tank3 Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Tank3` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if Tank3_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Tank3 High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Tank3` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in Tank3_Status and Tank3_Low_Alarm[0] == 0:
					update_sql("UPDATE `Tank3` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in Tank3_Status and Tank3_High_Alarm[0] == 0:
					update_sql("UPDATE `Tank3` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")		
			...
			if oldSetPoint_Tank4 != line:
				...
				print("\033[28;0H                                                                                                                       ")
				print("\033[28;0H(" + now.strftime("%Y/%m/%d %H:%M:%S") + ") SetPoint_Tank4: %s,%s,%s,%s,%s,%s"%(Tank4Value_Low,Tank4Value_High,Tank4Pump_ON,Tank4Pump_OFF,Tank4MixPump_Enabled,Tank4_Status))
				#SetPoint_Tank4
				update_sql("UPDATE `Tank4` SET Low = " + Tank4Value_Low + ", High = " + Tank4Value_High + ", Tank4Pump_ON = " + Tank4Pump_ON + ", Tank4Pump_OFF = " + Tank4Pump_OFF + ", Tank4MixPump_Enabled =" + Tank4MixPump_Enabled + ", Status ='" + Tank4_Status + "'")
				Tank4_Low_Alarm = fetch_sql("SELECT Low_Alarm FROM Tank4")
				Tank4_High_Alarm = fetch_sql("SELECT High_Alarm FROM Tank4")
				if Tank4_Low_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Tank4 Low",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Tank4` SET Low_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if Tank4_High_Alarm[0] == 1:
					if email_message_tls(login_address,email_password,to_address,"Tank4 High",smtp_server, smtp_port) == 1:
						update_sql("UPDATE `Tank4` SET High_Alarm = 2, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'LOW' in Tank4_Status and Tank4_Low_Alarm[0] == 0:
					update_sql("UPDATE `Tank4` SET Low_Alarm = 1, Low_Time = '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")
				if 'HIGH' in Tank4_Status and Tank4_High_Alarm[0] == 0:
					update_sql("UPDATE `Tank4` SET High_Alarm = 1, High_Time= '" + now.strftime("%b %d %Y %I:%M:%S %p") + "'")						
...
device_path='/dev/ttyUSByb2'    #override device_path (no user input)
