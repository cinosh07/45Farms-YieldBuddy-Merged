...
def drawInterface():
	...
	print("\033[3;78Hyieldbuddy2 v1.17a")
...
def email_message_tls(login_address,email_password,to_address,message,smtp_server,smtp_port):
	try:
		...
		headers = ["From: " + login_address,
			   "Subject: yieldbuddy2 Alert from " + yieldbuddy2_name,
			   "To: " + to_address,
			   "MIME-Version: 1.0",
...
#			time.sleep(2)
	...
	#Read Serial Device and Update according to the information that was received.
		...
		#If the 'timesync' counter value goes over 20, then update the Raspberry Pi's time to be that of the Arduino's.
			...
			Sensors,pH1,pH2,Temp,RH,TDS1,TDS2,CO2,Light,Water,MagX,MagY,MagZ,TankTotal,Tank1,Tank2,Tank3,Tank4=line.split(",")
...
print 'yieldbuddy2 v1.17a\r\n'
app_path = str( os.path.dirname(os.path.realpath(__file__)) )+"/"
print 'Application Path: ' + app_path + '\n'


device_path='/dev/ttyUSByb1'    #override device_path (no user input)
...
db = lite.connect(app_path+'www/sql/yieldbuddy2.sqlite3', timeout=10)
...
proc = subprocess.Popen(["sudo fuser " + app_path + "www/sql/yieldbuddy2.sqlite3"], stdout=subprocess.PIPE, shell=True)
...
addMessageLog("Started yieldbuddy2. Priority 15.")
printMessageLog()
loopcounter = 0
